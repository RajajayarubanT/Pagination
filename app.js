...
const API_URL = "https://raw.githubusercontent.com/Rajavasanthan/jsondata/master/pagenation.json"
...
req.onload = async () => {
    ...
    let limit = 5;
    let pagination_length = response.length / limit
    let current_page_start = 1


    let result_Container = document.querySelector('.project-tabel-values')
    let pagination_Container = document.querySelector('.pagination-container')


    let makeResItem = (id, name, email) => {
        let container = document.createElement('tr')

        container.innerHTML = `
        <td class="project-tabel-item-id">${id}</td>
        <td class="project-tabel-item">${name}</td>
        <td class="project-tabel-item">${email}</td>
    `

        return container
    }

    let makePaginationButton = (value) => {
        let container = document.createElement('button')
        container.classList.add('pagination-button')
    ...
    for (let i = 0; i < limit; i++) {
        ...
        let container = makeResItem(item.id, item.name, item.email)
    ...
    for (let i = 0; i < limit; i++) {
        ...
        let container = makePaginationButton(i + 1)

        pagination_Container.appendChild(container)

        button_event(container)
    }
    ...
    function button_event(BTN) {
        ...
        BTN.addEventListener('click', (e) => {
            ...
            if (e.target.value == '« First') value = 1
            else if (e.target.value == 'Last »') value = pagination_length


            value = parseInt(value)


            let start_index = (limit * value) - limit

            result_Container.innerHTML = ``

            for (let i = start_index; i < (limit + start_index); i++) {
            ...
            let btn_nodes = Array.prototype.slice.call(pagination_Container.children);
            let index = btn_nodes.indexOf(BTN)

            changepaginationButton(value, index)
        })
    }

    let first_last = false

    function changepaginationButton(current_value, index) {

        // if (!first_last && current_value > 1) {


        //     let first_btn = makePaginationButton('« First')
        //     let last_btn = makePaginationButton('Last »')

        //     pagination_Container.insertBefore(first_btn, pagination_Container.children[0])
        //     pagination_Container.appendChild(last_btn)

        //     button_event(first_btn)
        //     button_event(last_btn)
        ...
        if ((index > 3 || index < 1)) {
            ...
            if (index > 3) {
                if (pagination_length > (limit + current_page_start)) current_page_start++;
                start_index = current_page_start
            }
            else if (index < 1) {
                if (current_page_start >= 1) current_page_start--;
                start_index = current_page_start
            }
